...
while True:
    ...
    if choice == "1":
        from error_based import error_based_sql_injection
        error_based_sql_injection(url)
    elif choice == "2":
        from union_based import union_based_sql_injection
        union_based_sql_injection(url)
    elif choice == "3":
        from authentication_bypass import authentication_bypass
        authentication_bypass(url)
    elif choice == "4":
        from boolean_based import boolean_based_sql_injection
        boolean_based_sql_injection(url)
    elif choice == "5":
        from time_based import time_based_sql_injection
        time_based_sql_injection(url)
    elif choice == "6":
        from multiple_urls import multiple_urls_sql_injection
